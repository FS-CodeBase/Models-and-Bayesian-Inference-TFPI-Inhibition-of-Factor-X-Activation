function [PRMS,Cmat1,Cmat2,LL_vec] = am_alg_restart(am_info)
% FUNCTION: AM_alg
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp([f_id,'/Sobol_AM_',dat_str,'_nonadpt',num2str(non_adapt_itrs),...
        'adpt',num2str(adapt_itrs),'_nprms',num2str(nprms_est)])

load([f_id,'/Sobol_AM_',dat_str,'_nonadpt',num2str(non_adapt_itrs),...
        'adpt',num2str(adapt_itrs),'_nprms',num2str(nprms_est)],...
        'PRMS','Cmat1','Cmat2','LL_vec','am_info','itr');
curr_itr = itr;
Vn       = sp*cov(PRMS(1:curr_itr,:)).*ZeroMat;
LLpre    = LL_vec(curr_itr);
PrmsPre  = PRMS(curr_itr,:);

% Step 1: NON-ADAPTIVE METROPOLIS STEP
% accept = 0;
tic
if curr_itr < non_adapt_itrs
for itr = curr_itr+1:(non_adapt_itrs)
...
if curr_itr < non_adapt_itrs+mod_adapt
    % Compute covariance from non-adaptive step
    Cmat1 = cov(PRMS(1:itr,:));
    Cmat2 = Cmat1;
    Vn    = (sp*Cmat1).*ZeroMat;
    ...
    %         accept = accept + 1;    
        ...
        if mod(itr-non_adapt_itrs,mod_adapt)==0
            ...
            save([f_id,'/Sobol_AM_',dat_str,'_nonadpt',num2str(non_adapt_itrs),...
                    'adpt',num2str(adapt_itrs),'_nprms',num2str(nprms_est)],...
...
if curr_itr < non_adapt_itrs+2*mod_adapt
...
if curr_itr < non_adapt_itrs+adapt_itrs
