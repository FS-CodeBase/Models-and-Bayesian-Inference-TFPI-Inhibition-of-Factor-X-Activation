function [PrmsTmp,LLtmp] = rnd_prm_sample_am_two(LL_,mu_vec,prms_info,Cn,lb,ub)
...
% be negative
KM = fun_get_KM;
PrmsTmpMat = fun_get_full_paramset(abs(mvnrnd(mu_vec,Cn,10)),prms_info);
...
while isempty(PrmsTmpMat)
    PrmsTmpMat = [PrmsTmpMat;fun_get_full_paramset(abs(mvnrnd(mu_vec,Cn,10)),prms_info)];
    PrmsTmpMat = PrmsTmpMat(not(any(PrmsTmpMat<lb,2)),:);
    PrmsTmpMat = PrmsTmpMat(not(any(PrmsTmpMat>ub,2)),:);
    % fprintf('Num Prms: %d\n',size(PrmsTmpMat,1))
...
%   disp('IN WHILE')
    if count > 1E6
        warning('Had a problem generating samples AM P2! Count: %d',count)
